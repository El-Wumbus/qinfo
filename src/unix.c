 ...
 */
static char *get_board_name(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
  ...
  if (fp == NULL) {
    fprintf(stderr,
            "Error: couldn't open '/sys/devices/virtual/dmi/id/board_name'\n");
    return ("(ERR)");
  }
 ...
 */
static char *get_board_vendor(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
  ...
  if (fp == NULL) {
    fprintf(stderr,
            "Failed to get '/sys/devices/virtual/dmi/id/board_vendor'\n");
    return "(ERR)";
  }
...
static packagecount get_num_packages_dpkg(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
...
static packagecount get_num_packages_apk(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
...
static packagecount get_num_packages_pacman(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
...
static packagecount get_num_packages_flatpak(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
...
static packagecount get_num_packages_snap(void) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
...
static struct statvfs df(const char *path) {
  struct statvfs stat;
...
unsigned short get_disk_usage(char **dest, bool gigs) {
  ...
  float totalbytes = info.f_bsize * info.f_blocks,
        availablebytes = (info.f_bsize * info.f_bavail),
        usedbytes = totalbytes - availablebytes;
  signed int usageprecentage =
      roundf(100 * (((float)totalbytes - availablebytes) /
                    (((float)totalbytes + availablebytes) / 2.0)));
  float total, used;
  ...
  usedbytes = totalbytes - availablebytes;
  usageprecentage = round(100 * (((float)totalbytes - availablebytes) /
                                 (((float)totalbytes + availablebytes) / 2.0)));
